...
out_dir = '/scratch/10992/liaorunlong93/logs/nanoGPT_CHN_3G/chinese-image-discrete-rf-' + date
log_interval = 1
eval_iters = 200
...
def get_batch(split):
    ...
    y_ids = torch.stack([torch.from_numpy((data[i:i+block_size]).astype(np.int64)) for i in ix])

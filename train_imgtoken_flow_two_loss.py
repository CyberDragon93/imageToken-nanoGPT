...
from model_imgtoken_flow_two_loss import GPTConfig, GPT
...
out_dir = '/work/11018/jian03/vista/results/nano/chinese-image' + date
log_interval = 1
eval_iters = 200
...
# -----------------------------------------------------------------------------
use_ce_loss = True        # 是否启用 cross entropy 分支
rf_loss_weight = 1.0      # RF loss 权重
ce_loss_weight = 1.0      # CE loss 权重
...
def get_batch(split):
    ...
    if image_dataset is not None:
        x = image_dataset[x_ids]           # (B, T, C, H, W)
        y_images = image_dataset[y_ids]    # (B, T, C, H, W)
        y_classes = y_ids                  # (B, T)
    else:
        x, y_images, y_classes = x_ids, y_ids, y_ids

    if device_type == 'cuda':
        x = x.contiguous().pin_memory().to(device, non_blocking=True)
        y_images = y_images.contiguous().pin_memory().to(device, non_blocking=True)
        y_classes = y_classes.contiguous().pin_memory().to(device, non_blocking=True)
    else:
        x, y_images, y_classes = x.to(device), y_images.to(device), y_classes.to(device)

    return x, (y_images, y_classes.long())
    ...
    for split in ['train', 'val']:
        ...
        for _ in range(eval_iters):
            ...
            with accelerator.autocast():
                _, loss, aux = model(X, targets)
            tot.append(loss.item())
            rf.append(float(aux.get('rf_loss', torch.tensor(0.0)).item()) if isinstance(aux.get('rf_loss', None), torch.Tensor) else float(aux.get('rf_loss', 0.0)))
            ce.append(float(aux.get('ce_loss', torch.tensor(0.0)).item()) if isinstance(aux.get('ce_loss', None), torch.Tensor) else float(aux.get('ce_loss', 0.0)))
        out[split] = float(np.mean(tot))
        out[split + '_rf'] = float(np.mean(rf)) if len(rf) else float('nan')
        out[split + '_ce'] = float(np.mean(ce)) if len(ce) else float('nan')
    model.train()
    return out
...
while True:
    ...
    if iter_num != 0 and iter_num % eval_interval == 0:
        ...
        if accelerator.is_main_process:
            print(
                f"step {iter_num}: "
                f"train total {losses['train']:.4f} (rf {losses.get('train_rf', float('nan')):.4f}, ce {losses.get('train_ce', float('nan')):.4f}) | "
                f"val total {losses['val']:.4f} (rf {losses.get('val_rf', float('nan')):.4f}, ce {losses.get('val_ce', float('nan')):.4f})"
            )
            if wandb_log:
                wandb.log({
                    "iter": iter_num,
                    "val/loss": losses['val'],
                    "val/rf_loss": losses.get('val_rf', float('nan')),
                    "val/ce_loss": losses.get('val_ce', float('nan')),
                    "train/loss_eval": losses['train'],
                    "train/rf_loss_eval": losses.get('train_rf', float('nan')),
                    "train/ce_loss_eval": losses.get('train_ce', float('nan')),
                    "lr": lr,
                    "mfu": running_mfu*100
                })
    ...
    with accelerator.accumulate(model):
        for micro_step in range(eff_gas):
            ...
            with accelerator.autocast():
                logits, loss, aux = model(X, targets)
            total_micro_loss += loss.item()
            if aux.get('rf_loss', None) is not None:
                rf_loss_acc += (aux['rf_loss'].item() if isinstance(aux['rf_loss'], torch.Tensor) else float(aux['rf_loss']))
            if aux.get('ce_loss', None) is not None:
                ce_loss_acc += (aux['ce_loss'].item() if isinstance(aux['ce_loss'], torch.Tensor) else float(aux['ce_loss']))
    ...
    if (iter_num % log_interval == 0) and accelerator.is_main_process:
        ...
        rf_lossf = rf_loss_acc / eff_gas if rf_loss_acc > 0 else 0.0
        ce_lossf = ce_loss_acc / eff_gas if ce_loss_acc > 0 else 0.0
        ...
        print(f"iter {iter_num}: total {lossf:.4f} | rf {rf_lossf:.4f} | ce {ce_lossf:.4f} | {dt*1000:.2f}ms | mfu {running_mfu*100:.2f}%")

        if wandb_log:
            wandb.log({
                "iter": iter_num,
                "train/loss": lossf,
                "train/rf_loss": rf_lossf,

...
class GPTConfig:
    ...
    # add config for cross entropy loss
    use_ce_loss: bool = True
    rf_loss_weight: float = 1.0
...
class GPT(nn.Module):
    ...
    def forward(self, idx, targets=None):
        ...
        # parse target
        target_images, target_ids = None, None
        ...
        if self.config.use_ce_loss:
            ...
            if target_ids is not None:
                ...
                ce_loss = F.cross_entropy(
                    logits.view(B * T, V),
                    target_ids.view(B * T).to(device=logits.device, dtype=torch.long),
                    ignore_index=-1
                )
        
        if self.config.use_rf_loss:
            if target_images is None:
                raise ValueError("use_rf_loss=True but target_images is None.")
            # (B*T*repeat, P)
            target_flat = target_images.view(B * T, -1).repeat(self.config.rf_repeat, 1)
            z = x.view(B * T, -1).repeat(self.config.rf_repeat, 1)
            rf_loss = self.rf_loss_model(target_flat, z)
        
        losses = []
        if rf_loss is not None:
            losses.append(self.config.rf_loss_weight * rf_loss)
        if ce_loss is not None:
            losses.append(self.config.ce_loss_weight * ce_loss)
